...
Use the `ue5_agent_blueprint_tools` MCP server. The package or legacy config may also be referred to as `ue5-agent-blueprint-tools-mcp`; both names refer to the same AgentBlueprintTools MCP bridge. Never edit `.uasset` files directly.
...
2. Use the returned material outputs, reachable node counts, expression inputs/outputs, texture samples, function calls, and key parameters to summarize the material intent before editing logic.
3. Preserve required blend mode and visual contract; do not switch translucent VFX materials to opaque or masked unless the user explicitly accepts that visual change.
4. Prefer replacing or bypassing a specific high-cost reachable branch over rebuilding the whole graph. Keep the old branch only when useful as an unconnected reference; delete only known-dead expressions.
5. Use `patch_material` with a small material patch.
6. Recompile material after edits.
...
For one specific asset, pass `assetPaths: ["/Game/.../AssetName"]` instead of scanning all `/Game`.

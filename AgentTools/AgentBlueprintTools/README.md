...
skills/ue5-blueprint-agent/          可分发的 Codex Skill
...
推荐把 UE 插件和 agent 工具分开放：

```text
YourProject/
  Plugins/AgentBlueprintTools/
  AgentTools/AgentBlueprintTools/
```
...
```bash
codex mcp add ue5_agent_blueprint_tools -- node AgentTools/AgentBlueprintTools/mcp-server/server.js
```
...
```bash
cd AgentTools
npm --workspace ue5-agent-blueprint-tools-mcp run inspect
```
